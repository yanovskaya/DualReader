...
🧠 **Инициализаторы полей и `updatable=false`:**
...
Здесь **два разных механизма**, которые часто путают:

- `= Instant.now()` — это **Java**-инициализатор. Срабатывает **когда мы делаем `new Book()`**. То есть «время создания объекта в памяти», а не «время вставки в БД». Если объект провалялся в памяти 5 минут до `save()` — в БД попадёт момент создания, а не момент сохранения. Для нашей задачи разница неважна, но знать стоит.
- `updatable = false` — Hibernate при следующих `UPDATE` **не будет** включать эту колонку в SQL. То есть случайно перезаписать `created_at` через `book.setCreatedAt(...)` + `save()` не получится. **Первичный `INSERT` это не блокирует** — там поле уйдёт в БД нормально. `updatable` действует только на UPDATE-запросы.

Альтернатива — не задавать значение в Java и положиться на `DEFAULT NOW()` в SQL-схеме. Тогда нужно `insertable = false` или дёргать `refresh()` после `save()`, чтобы прочитать значение, которое поставила БД. Наш способ проще.

...
### `Map.class` — class-токен

```java
mapper.readValue(response.body(), Map.class);
```

`Map.class` — это **объект типа `Class<Map>`**, который представляет сам класс `Map` как значение. Jackson смотрит на него и думает: «ага, нужно распарсить JSON именно в `Map`».

Любой класс в Java имеет такое поле `.class`: `String.class`, `Integer.class`, `Book.class`. Это не вызов метода — это литерал, как `42` или `"hello"`. Аналогов в JS нет, в TS — частично `typeof MyClass`, но не то же самое.

Если хочется типизировано, есть `mapper.readValue(json, new TypeReference<Map<String, Object>>(){})` — тогда Jackson сохранит дженерик-параметры. Мы упрощаем.

### `RuntimeException(..., e)` — цепочка исключений

```java
throw new RuntimeException("OpenAI API call failed: " + e.getMessage(), e);
```

У `Exception` есть второй конструктор-аргумент — `Throwable cause`. Когда мы перехватываем низкоуровневое исключение и бросаем своё, **передавай оригинал как `cause`** — так он не потеряется.

Когда выше по стеку кто-то напечатает stack trace, увидит и наше понятное сообщение, и под ним «Caused by: java.net.ConnectException: Connection refused». Без второго аргумента причина теряется, и отладка превращается в гадание.

Правило: если ловишь `Exception` чтобы перебросить в виде `RuntimeException` — **всегда** передавай `e` вторым аргументом.

...
### Зачем перечитывать книгу из БД в цикле
...
Между батчами проходит несколько секунд (вызов OpenAI). За это время:
- кто-то мог **удалить книгу** (`DELETE /books/:id`);
- кто-то мог **изменить** её (например, поменять `translation_status` руками в БД).

Если бы мы продолжали работать со «старым» объектом `book`, то могли бы перезаписать чужие изменения или сохранить запись, которой уже нет в БД. `findById(...).orElse(book)` — мягкий fallback: «возьми свежую версию, если есть; если нет — продолжай со старой».

Это не идеальная стратегия (правильнее было бы остановить перевод, если книга удалена), но для нашего проекта — норм.

...
## 🤔 Проверь себя
  ...
  Сейчас ключ кэша — это (`word`, `source_lang`, `target_lang`). Что произойдёт, если пользователь спросит «put» с контекстом «I can put up with this»? Должен ли контекст быть частью ключа?

  **Ответ:** в нашем случае — **нет**. Мы храним базовую форму слова/фразового глагола. Контекст помогает OpenAI выбрать правильное значение, но результат кэшируется по итоговому слову. Если бы мы хотели разные переводы для разных контекстов — пришлось бы добавить хэш контекста в ключ.
  </details>
